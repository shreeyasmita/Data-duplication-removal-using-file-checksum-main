    ...
    if request.method == 'POST':
        ...
        for i in all_files:
            if str(i.hash)==hash:
                # Found duplicate!
                duplicate_file = i
                # Find all uploads using this hash
                existing_uploads = Uploads.objects.filter(u_hash=i)
                break
        
        if duplicate_file:
            # Duplicate detected - delete the temporary file and show warning
            os.remove(t_file)
            temUploads.objects.all().delete()
            
            # Create message with existing file info
            existing_titles = ', '.join([up.u_title for up in existing_uploads])
            ...
            messages.warning(request, 
                f'⚠️ DUPLICATE FILE DETECTED! This file already exists as: "{existing_titles}". '
                f'File not uploaded to save storage space ({format_size(file_size)} saved). '
                f'You can view or delete the existing file from your uploads.')
            
            # Redirect based on user role
            if request.user.groups.filter(name='admin').exists():
                return redirect('admin', pk)
            else:
                return redirect('user', pk)

        # Not a duplicate - save normally
        venue = Files(hash=hash, path=str(t_file), file_size=len(readfile))
        venue.save()
        ...
        print("New unique file uploaded successfully")
        
        messages.success(request, f'✅ File "{t_title}" uploaded successfully! This is a unique file.')
...
def adminprofile(request,pk):
    ...
    admin_uploads = Uploads.objects.filter(uploaded_by__name=str(request.user))
    admin_shares = Shared.objects.filter(shared_to__name=str(request.user))

    # Calculate statistics
    total_files = Files.objects.count()
    total_uploads = Uploads.objects.count()
    total_unique_files = total_files
    total_duplicate_references = total_uploads - total_files if total_uploads > total_files else 0
    
    # Calculate storage saved by deduplication
    all_files = Files.objects.all()
    total_actual_storage = sum(f.file_size for f in all_files)
    
    # Calculate what storage would be without deduplication
    storage_would_be = 0
    for upload in Uploads.objects.all():
        if upload.u_hash:
            storage_would_be += upload.u_hash.file_size
    
    storage_saved = storage_would_be - total_actual_storage if storage_would_be > total_actual_storage else 0
    ...
    stats = {
        ...
        'duplicate_references': total_duplicate_references,
        'storage_saved': format_bytes(storage_saved),
        'storage_saved_bytes': storage_saved,
        'total_storage': format_bytes(total_actual_storage),
        'total_users': A_U.objects.filter(groups__name='user').count(),
        'total_shares': shares.count(),
    }

    context = {'upload_filters': myfilter, 'shares': shares, 'users':users, 'uploaded_files': uploads, 'mysharefilter': mysharefilter,
               'usersfilter' : myuserfilter,'blocked_users':blocked_users, 'stats': stats, 'admin_uploads': admin_uploads, 'admin_shares': admin_shares }
